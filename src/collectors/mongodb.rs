...
fn format_doc_json(doc: &Document) -> String {
...
async fn fetch_all(uri: &str, state: &Arc<Mutex<AppState>>) -> anyhow::Result<()> {
    ...
    for db_name in databases {
        ...
        let mut colls = vec![];
        if let Ok(Ok(collections)) = tokio::time::timeout(Duration::from_secs(5), db.list_collection_names()).await {
            colls = collections;
        }
        ...
        for coll_name in colls.into_iter().take(5) {
            ...
            if let Ok(Ok(mut cursor)) = tokio::time::timeout(Duration::from_secs(5), coll.find(mongodb::bson::doc! {})).await {
                let mut count = 0;
                while let Some(doc_res) = cursor.next().await {
                    let doc_res: Result<Document, mongodb::error::Error> = doc_res;
                    if let Ok(d) = doc_res {
                        docs.push(format_doc_json(&d));
                        count += 1;
                        if count >= 20 { break; } 
                    } else {
                        break;
